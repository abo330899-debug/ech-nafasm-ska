...
- **Static-site media (R2 replacement)**: Cloudflare R2 was disabled account-wide (2026-07-21), so the static hosts (Cloudflare Pages + GitHub Pages) now load ALL media + content.json from the Replit deployment via token-gated public routes `/api/pub/<NAFSAM_STATIC_MEDIA_TOKEN>/{media,posters,images}/*` and `/content.json` (`artifacts/api-server/src/routes/pub.ts`). `VITE_R2_BASE` in both `.env.cloudflare-pages` files points at `https://ech-memories-ska-love.replit.app/api/pub/<token>`. Env `NAFSAM_STATIC_MEDIA_TOKEN` (shared, ≥16 chars, fail-closed) — rotating it requires updating both env files + pushing. **The Replit deployment must be published with PUBLIC visibility** or static-site media breaks.
