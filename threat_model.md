...
- As of 2026-07-21 the R2 bucket is retired (account-wide R2 disable). The static hosts now load the FULL private tree (`media`, `posters`, `images`) plus `content.json` from the Replit deployment through the token-gated public routes `/api/pub/:token/*` (`artifacts/api-server/src/routes/pub.ts`). The token (`NAFSAM_STATIC_MEDIA_TOKEN`) is a dedicated random value, fail-closed, constant-time compared, never derived from login passwords, and is intentionally baked into the public static bundles and the committed `.env.cloudflare-pages` files — the same public-by-obscurity posture as the old public r2.dev base URL, but now a superset (entire private tree, not a curated upload subset). This requires the Replit deployment to be published with PUBLIC visibility, which exposes `/api/auth/login` (rate-limited by trusted client IP), `/api/private/*` (cookie-auth), and `/api/reorder` (fails closed without `NAFSAM_ADMIN_TOKEN`) to the open internet. The token appears in request URLs and therefore in server logs.
